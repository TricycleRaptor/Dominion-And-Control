...
function ENT:Initialize()
	...
	-- Spawn the clamp.
	local entBarricade = ents.Create('prop_physics')
	entBarricade:SetModel('models/props_combine/combine_barricade_short01a.mdl')
	entBarricade:SetPos(self:GetPos() + Vector(0, 0, 30))
	entBarricade:SetAngles(self:GetAngles()+Angle(0,180,0))
	entBarricade:Spawn()

	if IsValid(entBarricade:GetPhysicsObject()) then
		constraint.Keepupright(entBarricade, entBarricade:GetAngles(), 0, 1000) -- Keep upright after spawning
		--entBarricade:GetPhysicsObject():EnableMotion(false) -- Freeze in place after spawning
	end	

	self:SetParent(entBarricade) -- parent it to the clamp.
	self:SetAngles(entBarricade:GetAngles())
	self:DeleteOnRemove(entBarricade)

	timer.Create("DAC.AR3VectorAdjustment_" .. tostring(self), 0.01, 1, function() -- Because parenting at the time of spawning fucks it up
